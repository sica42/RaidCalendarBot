...
import raidcalbot.common.{RaidCalBotConfig, WowExpansion}
...
trait GamePackets {
  ...
  object ChatEvents {
    // err...
    lazy val CHAT_MSG_SAY = if (RaidCalBotConfig.getExpansion == WowExpansion.Vanilla) 0x00.toByte else 0x01.toByte
    lazy val CHAT_MSG_GUILD = if (RaidCalBotConfig.getExpansion == WowExpansion.Vanilla) 0x03.toByte else 0x04.toByte
    lazy val CHAT_MSG_OFFICER = if (RaidCalBotConfig.getExpansion == WowExpansion.Vanilla) 0x04.toByte else 0x05.toByte
    lazy val CHAT_MSG_YELL = if (RaidCalBotConfig.getExpansion == WowExpansion.Vanilla) 0x05.toByte else 0x06.toByte
    lazy val CHAT_MSG_WHISPER = if (RaidCalBotConfig.getExpansion == WowExpansion.Vanilla) 0x06.toByte else 0x07.toByte
    lazy val CHAT_MSG_EMOTE = if (RaidCalBotConfig.getExpansion == WowExpansion.Vanilla) 0x08.toByte else 0x0A.toByte
    lazy val CHAT_MSG_TEXT_EMOTE = if (RaidCalBotConfig.getExpansion == WowExpansion.Vanilla) 0x09.toByte else 0x0B.toByte
    lazy val CHAT_MSG_CHANNEL = if (RaidCalBotConfig.getExpansion == WowExpansion.Vanilla) 0x0E.toByte else 0x11.toByte
    lazy val CHAT_MSG_SYSTEM = if (RaidCalBotConfig.getExpansion == WowExpansion.Vanilla) 0x0A.toByte else 0x00.toByte
    lazy val CHAT_MSG_CHANNEL_JOIN = if (RaidCalBotConfig.getExpansion == WowExpansion.Vanilla) 0x0F.toByte else 0x12.toByte
    lazy val CHAT_MSG_CHANNEL_LEAVE = if (RaidCalBotConfig.getExpansion == WowExpansion.Vanilla) 0x10.toByte else 0x13.toByte
    lazy val CHAT_MSG_CHANNEL_LIST = if (RaidCalBotConfig.getExpansion == WowExpansion.Vanilla) 0x11.toByte else 0x14.toByte
    lazy val CHAT_MSG_CHANNEL_NOTICE = if (RaidCalBotConfig.getExpansion == WowExpansion.Vanilla) 0x12.toByte else 0x15.toByte
    lazy val CHAT_MSG_CHANNEL_NOTICE_USER = if (RaidCalBotConfig.getExpansion == WowExpansion.Vanilla) 0x13.toByte else 0x16.toByte
    lazy val CHAT_MSG_HARDCORE = 0x5E.toByte

    lazy val CHAT_MSG_ACHIEVEMENT = if (RaidCalBotConfig.getExpansion == WowExpansion.MoP) 0x2E.toByte else 0x30.toByte
    lazy val CHAT_MSG_GUILD_ACHIEVEMENT = if (RaidCalBotConfig.getExpansion == WowExpansion.MoP) 0x2F.toByte else 0x31.toByte
  ...
  object GuildEvents {
    ...
    val GE_PROMOTED = if (RaidCalBotConfig.getExpansion == WowExpansion.Cataclysm) 0x01 else 0x00
    val GE_DEMOTED = if (RaidCalBotConfig.getExpansion == WowExpansion.Cataclysm) 0x02 else 0x01
    val GE_MOTD = if (RaidCalBotConfig.getExpansion == WowExpansion.Cataclysm) 0x03 else 0x02
    val GE_JOINED = if (RaidCalBotConfig.getExpansion == WowExpansion.Cataclysm) 0x04 else 0x03
    val GE_LEFT = if (RaidCalBotConfig.getExpansion == WowExpansion.Cataclysm) 0x05 else 0x04
    val GE_REMOVED = if (RaidCalBotConfig.getExpansion == WowExpansion.Cataclysm) 0x06 else 0x05
    val GE_SIGNED_ON = if (RaidCalBotConfig.getExpansion == WowExpansion.Cataclysm) 0x10 else 0x0C
    val GE_SIGNED_OFF = if (RaidCalBotConfig.getExpansion == WowExpansion.Cataclysm) 0x11 else 0x0D
  }
  ...
  object ChatChannelIds {
    ...
    val GUILD_RECRUITMENT = RaidCalBotConfig.getExpansion match {
