...
import raidcalbot.game.warden.{WardenHandler, WardenPackets}
import raidcalbot.bot.BotCommandHandler
import com.typesafe.scalalogging.StrictLogging
import io.netty.buffer.{ByteBuf, PooledByteBufAllocator}
import io.netty.channel.{ChannelFuture, ChannelHandlerContext, ChannelInboundHandlerAdapter}
//import raidcalbot.commands.{CommandHandler, WhoResponse}
...
case class Currency(gold: Int, silver: Int, copper: Int) {
  override def toString: String = f"${gold}g${silver}s${copper}c"
}
...
*/
  ...
  override def sendAddonMessageToWow(tp: Byte, message: String, prefix: String): Unit = {
    ctx.fold(logger.error("Cannot send message! Not connected to WoW!"))(ctx => {
      ctx.writeAndFlush(buildAddonMessage(tp, message.getBytes("UTF-8"), prefix.getBytes("UTF-8")))
    })
  }

  protected def buildAddonMessage(tp: Byte, utf8MessageBytes: Array[Byte], utf8PrefixBytes: Array[Byte]): Packet = {
    val out = PooledByteBufAllocator.DEFAULT.buffer(128, 8192)
    out.writeIntLE(tp)
    out.writeIntLE(-1)
    out.writeBytes(utf8PrefixBytes);
    out.writeByte(9);
    out.writeBytes(utf8MessageBytes)
    out.writeByte(0)
...
//      case SMSG_WHO => handle_SMSG_WHO(msg)
  ...
  protected def parseAuthChallenge(msg: Packet): AuthChallengeMessage = {
    ...
    out.writeShortLE(0)
    out.writeIntLE(RaidCalBotConfig.getGameBuild)
    out.writeIntLE(0)
  ...
  private def handle_SMSG_LOGIN_VERIFY_WORLD(msg: Packet): Unit = {
    ...
    inWorld = true
    Global.discord.changeRealmStatus(s"${Global.config.wow.character} on $realmName")
...
*/
  ...
  protected def sendChatMessage(chatMessage: ChatMessage): Unit = {
    if (chatMessage.tp == ChatEvents.CHAT_MSG_WHISPER || chatMessage.tp == ChatEvents.CHAT_MSG_GUILD) {  
  ...
  protected def parseChatMessage(msg: Packet): Option[ChatMessage] = {
    ...
    var channelName = if (tp == ChatEvents.CHAT_MSG_CHANNEL) {
    ...
    // We only care about whisper and addon guild amessages
    if (tp != ChatEvents.CHAT_MSG_WHISPER && (tp != ChatEvents.CHAT_MSG_GUILD || lang != -1)) {
      return None
    }
    ...
    var txt = msg.byteBuf.readCharSequence(txtLen - 1, Charset.forName("UTF-8")).toString

    if (lang == -1) {
      val i = txt.indexOf("\t")
      if (i > -1) {
        val addon = txt.substring(0, i)
        val message = txt.substring(i + 1)

        if (addon == "RaidCal") {
          channelName = Some("Addon")
  ...
  protected def parseWhoResponse(msg: Packet): Seq[WhoResponse] = {
    ...
    if (displayCount == 0) {
      ...
      (0 until displayCount).map(i => {
        ...
        val gender = if (RaidCalBotConfig.getExpansion != WowExpansion.Vanilla) {
