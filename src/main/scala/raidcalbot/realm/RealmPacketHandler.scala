...
class RealmPacketHandler(realmConnectionCallback: RealmConnectionCallback)
  ...
  override def channelActive(ctx: ChannelHandlerContext): Unit = {
    ...
    this.ctx = Some(ctx)
    val version = RaidCalBotConfig.getVersion.split("\\.").map(_.toByte)
    ...
    if (RaidCalBotConfig.getExpansion == WowExpansion.Vanilla) {
      byteBuf.writeByte(3)
    } else {
      byteBuf.writeByte(8)
    }
    ...
    byteBuf.writeShortLE(RaidCalBotConfig.getRealmBuild)
  ...
  private def handle_CMD_AUTH_LOGON_CHALLENGE(msg: Packet): Unit = {
    ...
    md.update(aArray)
    md.update(buildCrcHashes.getOrElse((RaidCalBotConfig.getRealmBuild, Global.config.wow.platform), new Array[Byte](20)))
